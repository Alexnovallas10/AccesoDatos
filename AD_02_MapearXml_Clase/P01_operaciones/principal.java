...
public class principal {
	...
	public static void visualizarxml() throws IOException, JAXBException {
		...
		System.out.println("------------------------------ ");
			...
			JAXBElement jaxbElement = (JAXBElement) u.unmarshal(new FileInputStream("./AD_02_MapearXml_Clase/ventasarticulos.xml"));
	...
	private static void insertarventa(int numeventa, String nomcli, int uni, String fecha) {
		...
		try {
			...
			if (existe == 0) {
				...
				Marshaller m = jaxbContext.createMarshaller();
				m.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, Boolean.TRUE);
				m.marshal(jaxbElement, new FileOutputStream("./AD_02_MapearXml_Clase/ventasarticulos.xml"));
